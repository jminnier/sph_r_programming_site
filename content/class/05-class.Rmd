...
<iframe width="560" height="315" src="https://www.youtube.com/embed/RvQfESoJwjI" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
# with duplicate rows in the left and right data, we end up with duplicated rows in the full data
...
# perhaps the example_cohort was a mistake, and we want to remove those duplicated rows first:
...
# what if we weren't supposed to have two juan values in the "example_other" tibble
# let's assume we only want to have the lowest y value in example_other for each name
example_other %>%
  group_by(name) %>%
  slice_min(y)
...
In part 6 I have used a "real" data example, basically this is the most common type of primary data that I receive as a statistician -- and excel sheet where someone in a lab has input data, copied some omics data from another excel sheet into that table, and sent it to me via email. This type of data can lend itself to lots of strange formatting issues, and also lots of data input error.

I also need to teach you the basic tools for cleaning and manipulating data before I can get you started with "real" data in super messy format, because otherwise it might be overwhelming doing a series of steps! But I think you're almost ready! You'll see how the data cleaning steps can really pile on today during the modestly messy example. Most of my "data cleaning" or "importing" or "wrangling" tasks as a statistician involve dealing with weird spreadsheet formats, typos, missing values, and then creating new variables that I need for analyses or graphing. I think you've learned most of those tasks by now in this class.

If anyone has primary data they would like me to go over in class (and can share it), please send it to me!
